...
class Migration:
    ...
    def forwards(self, orm):
        Language.objects.get_or_create(short='?', full='unknown')

        for lang in LANG_CODE:
            Language.objects.get_or_create(short=lang[0], full=lang[1])

        for book in Book.objects.filter(language__short='?'):
            if book.lang:
                try:
