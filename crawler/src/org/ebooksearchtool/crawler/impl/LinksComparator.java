...
class LinksComparator implements Comparator<URI> {
    ...
    String[] badSites = new String[] {"facebook", "wikipedia", "tumblr"};
    
    private int linkValue(URI uri) {
        String s = uri.toString();
        for (String badSite : badSites) {
            if (s.indexOf(badSite) >= 0) return -10;
        }
        if (uri.toString().indexOf("book") >= 0) return 10;
        if (uri.toString().indexOf("epub") >= 0) return 5;
        return 0;
    }
